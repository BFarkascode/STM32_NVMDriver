...
Additinal note: ALL IRQs must be deactivated when a half-page burst is running, otherwise the mcu freezes. This is not indicated anywhere within the refman. Also, I had trouble using pointers within the half-page burst writing. It has been a lot more reliable to pass the actual data buffer as an extern directly into the burst function using a loop. Similarl to the erase function, we don't need to step the FLASH addresses.
