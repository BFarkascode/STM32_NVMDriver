...
For the sake of simplicity, the push button is connected directly to then external interrupt. The code replacement functions are placed within the IRQ itself (which is not a good practice, but since we aren't doing anything else within the code, we don't care). The interrupt is activated on the L053r* nulceo board by pushing the blue button.

On pushing the button, the blinking speed of the inbuilt LED should change (and then, unlike doing this by modifying a variable, retain that change upon reset).
